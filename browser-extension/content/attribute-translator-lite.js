(() => {
  ...
  function attributeNames(element) {
    ...
    if (element instanceof HTMLInputElement && ["button", "submit", "reset"].includes(element.type) && element.hasAttribute("value")) names.push("value");
    return names;
  }
  ...
  function scheduleFlush(delay = 150) {
    if (!enabled() || processing || flushTimer) return;
  ...
  function scan(root = document) {
    ...
    if (root.querySelectorAll) {
      for (const element of root.querySelectorAll(SELECTOR)) inspect(element);
    }
  }

  function queueRoot(root) {
    if (!enabled() || !root || root.nodeType !== Node.ELEMENT_NODE) return;
    if (root.dataset?.ftOwned === "1") return;
  ...
  function requeueFailed(item) {
    ...
    if (count > 3) return;
    setTimeout(() => enqueue(item.element, item.attr), Math.min(5000, 650 * (2 ** (count - 1))));
  }

  async function flush() {
    if (processing || !enabled() || !queue.size) return;
    processing = true;
    const items = [...queue.values()].slice(0, 24);
    ...
    try {
      ...
      processing = false;
      if (queue.size && enabled()) scheduleFlush(90);
    }
  }
    ...
    observer = new MutationObserver(mutations => {
      ...
      for (const mutation of mutations) {
        ...
        for (const added of mutation.addedNodes) {
          if (added.nodeType === Node.ELEMENT_NODE) queueRoot(added);
        }
      }
    });
  ...
  async function loadSettings({ scanNow = true } = {}) {
    ...
    if (scanNow && enabled()) queueRoot(document.body);
  }

  function handleExclusionsChanged() {
    restoreAll();
    if (enabled()) queueRoot(document.body);
  }

  chrome.runtime.onMessage.addListener(message => {
    if (message?.type === "FT_SET_PAUSED") {
      paused = Boolean(message.paused);
      if (!paused && enabled()) queueRoot(document.body);
      return false;
    }
    if (message?.type === "FT_TRANSLATE_NOW") {
      paused = false;
      restoreAll();
      setTimeout(() => queueRoot(document.body), 180);
      return false;
    }
    if (message?.type === "FT_RESCAN_PAGE") {
      if (!paused) queueRoot(document.body);
      return false;
    }
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    if (area === "sync" && Object.keys(changes).some(key => key in DEFAULTS)) void loadSettings();
  });
  ...
  async function init() {
    ...
    await loadSettings({ scanNow: false });
